...
module("Unit | utils | rdfa-parser-test", () => {
  ...
  test("correct predicate node mapping", assert => {
    // language=XML
    const {root, elements: {mboxPred, mboxPred2, namePred, personNode}} = vdom`
    ...
    const {dataset, predicateToNodesMapping, nodeToPredicatesMapping} = RdfaParser.parse({
      modelRoot: root,
      baseIRI: "http://example.com"
    });
    ...
    // check name
    const nameMapping = predicateToNodesMapping.get("http://xmlns.com/foaf/0.1/name");
    if (!nameMapping) {
    ...
    // check mbox
    const mboxMapping = predicateToNodesMapping.get("http://xmlns.com/foaf/0.1/mbox");
    if (!mboxMapping) {
      throw new AssertionError();
    }
