...
import { preWrapToNormalWhiteSpace, normalToPreWrapWhiteSpace } from '@lblod/ember-rdfa-editor/utils/whitespace-collapsing';
...
module('Unit | utils | whitespace-collapsing | pre-wrap-to-normal ', () => {
  test('properly replaces spaces with non breaking ones', function(assert) {
    ...
    assert.strictEqual(result, `I like two spaces after my sentence.${NON_BREAKING_SPACE} See?`)
  });
  ...
  test('properly handles a text node only consisting of spaces', function (assert) {
    const editorText = '              ';
    const result = preWrapToNormalWhiteSpace(editorText);
    assert.strictEqual(result, `${NON_BREAKING_SPACE} ${NON_BREAKING_SPACE} ${NON_BREAKING_SPACE} ${NON_BREAKING_SPACE} ${NON_BREAKING_SPACE} ${NON_BREAKING_SPACE} ${NON_BREAKING_SPACE}${NON_BREAKING_SPACE}`);
  });
});
