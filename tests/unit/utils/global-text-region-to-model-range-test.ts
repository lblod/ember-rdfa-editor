...
module("Unit | Utility | global-offset-to-path", function () {
  test("returns the correct range for a simple text", function (assert) {
    // language=XML
    const {root, textNodes: {testNode}} = vdom`<div><text __id="testNode">abc</text></div>`;
    const calculatedRange = globalTextRegionToModelRange(root as ModelElement, 0, 3);
  ...
  test("returns the correct range with nested elements", function (assert) {
    // language=XML
    const {root, textNodes: {startNode, endNode}} = vdom`<div>
             <div>
    ...
    const calculatedRange = globalTextRegionToModelRange(root as ModelElement, 0, 6);
  ...
  test("for the start of a region prefers the start of a textnode when possible", function (assert) {
    // language=XML
    const {root, textNodes: { preferedNode }} = vdom`<div>
           <div>
    ...
    const calculatedRange = globalTextRegionToModelRange(root as ModelElement, 3, 6);
  ...
  test("for a substring of a text node it calculates the correct range", function (assert) {
    const {root, textNodes: { theNode }} = vdom`<div><text __id="theNode">some text here</text></div>`;
    const calculatedRange = globalTextRegionToModelRange(root as ModelElement, 4, 8);
    const range = ModelRange.fromInTextNode(theNode, 4,8);
    assert.deepEqual(calculatedRange, range);
  });

  test("it correctly maps a void node to an offset of 1", function (assert) {
    const {root, textNodes: { theNode }} = vdom`<div><br /><text __id="theNode">some text here</text></div>`;
    const calculatedRange = globalTextRegionToModelRange(root as ModelElement, 1, 5);
  ...
  test("it correctly calculates the range when the region is spanning a void node", function (assert) {
    const {root, textNodes: { startNode, endNode }} = vdom`<div><text __id="startNode">some</text><br /><text __id="endNode">text here</text></div>`;
    const calculatedRange = globalTextRegionToModelRange(root as ModelElement, 0, 9);
    const startPosition = ModelPosition.fromInTextNode(startNode, 0);
