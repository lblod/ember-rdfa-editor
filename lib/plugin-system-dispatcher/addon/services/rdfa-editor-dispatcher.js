...
export default class RdfaEditorDispatcher extends Service {
  constructor(){
    super();
    this.pluginServices = A();
    for (let plugin of calculateAllPlugins( editorProfiles )) {
      const varNameForPlugin = variableNameForPlugin(plugin);
      // inject the plugin into this service
      set(this, varNameForPlugin, inject(plugin));
      this.pluginServices.pushObject( get(this, varNameForPlugin) );
    }
  }
