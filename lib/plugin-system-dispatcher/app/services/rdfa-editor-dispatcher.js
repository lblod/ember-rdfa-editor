...
import PluginEditorApi from '@lblod/ember-rdfa-editor/utils/document-interface';
...
export default class RdfaEditorDispatcher extends Service {
  ...
  */
    ...
    return new RSVP.Promise((resolve) => {
      ...
      if( plugins ){
        for (let plugin of plugins) {
          ...
          case "1.0":
            this.dispatchV1(plugin, pluginService, hintsRegistryIndex, contexts, hintsRegistry, editor, extraInfo);
            break;
          default:
            debug(`DEPRECATION: Plugin ${plugin} is using a deprecated editor api version (${pluginApiversion}).`);
            this.dispatchGenesis(plugin, pluginService, hintsRegistryIndex, contexts, hintsRegistry, editor, extraInfo);
            break;
          }
        }
      }
  ...
  dispatchGenesis(plugin, pluginService, hintsRegistryIndex, contexts, hintsRegistry, editor, extraInfo) {
  ...
  dispatchV1(plugin, pluginService, hintsRegistryIndex, contexts, hintsRegistry, editor, extraInfo) {
    const pluginEditorApi = new PluginEditorApi(editor);
    return this.dispatchGenesis(plugin, pluginService, hintsRegistryIndex, contexts, hintsRegistry, pluginEditorApi, extraInfo);
  }
