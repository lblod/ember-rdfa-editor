...
import {isInputEvent, isKeyUpEvent} from "@lblod/ember-rdfa-editor/editor/input-handlers/event-helpers";
...
export default class FallbackInputHandler extends InputHandler {
  ...
  isHandlerFor(event: Event): boolean {
    return (isKeyUpEvent(event) && this.FALLBACK_KEY_UP_KEYS.includes(event.key))
  ...
  handleEvent(event: Event): HandlerResponse {
    this.rawEditor.externalDomUpdate(`Uncaptured event of type ${event.type}, restoring editor state.`);
