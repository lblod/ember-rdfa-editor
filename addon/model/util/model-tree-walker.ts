...
export default class ModelTreeWalker<T extends ModelNode = ModelNode> implements Iterable<T> {
  ...
  previousNode(): ModelNode | null {
    ...
    while (node !== this.root) {
      ...
      while (sibling) {
        ...
        while (result !== FilterResult.FILTER_REJECT && ModelTreeWalker.hasChild(node)) {
          node = node.lastChild;
  ...
  nextNode(): ModelNode | null {
    ...
    while (true) {
      ...
      while (this.descend && result !== FilterResult.FILTER_REJECT && ModelTreeWalker.hasChild(node)) {
  ...
  private getStartNodeFromPosition(startPosition: ModelPosition): ModelNode | null {
  ...
  private traverseChildren(traverseType: TraverseType): ModelNode | null {
    ...
    let node = ModelTreeWalker.getChild(this._currentNode, traverseType);
    while (node) {
      ...
      if (result === FilterResult.FILTER_ACCEPT) {
        ...
        const child = ModelTreeWalker.getChild(node, traverseType);
        if (child) {
          node = child;
          continue;
        }
      ...
      while (node) {
        const sibling = ModelTreeWalker.getSibling(node, traverseType);
        if (sibling) {
          node = sibling;
          break;
        }
  ...
  private traverseSiblings(traverseType: TraverseType): ModelNode | null {
    ...
    // eslint-disable-next-line no-constant-condition
    while (true) {
      let sibling = ModelTreeWalker.getSibling(node, traverseType);
      while (sibling) {
        node = sibling;
        const result = this.filterNode(node);
        if (result === FilterResult.FILTER_ACCEPT) {
        ...
        sibling = ModelTreeWalker.getChild(node, traverseType);
        if (result === FilterResult.FILTER_REJECT || !sibling) {
          sibling = ModelTreeWalker.getSibling(node, traverseType);
        }
      }
  ...
  private static getSibling(node: ModelNode, traverseType: TraverseType): ModelNode | null {
  ...
  private static getChild(node: ModelNode, traverseType: TraverseType): ModelNode | null {
  ...
  private static hasChild(node: ModelNode): node is ModelElement {
