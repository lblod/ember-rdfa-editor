...
function buildTextNodeArrayWithOffsets(
  treeWalker: ModelTreeWalker
): ModelTextWithOffsets[] {
...
function findBestMatchingTextWithOffset(
  textNodes: ModelTextWithOffsets[],
  offset: number,
  preferredEdge: TextEdge
): ModelTextWithOffsets {
  const eligibleNodes = textNodes.filter((modelTextWithOffsets) => {
    return (
      modelTextWithOffsets.startOffset <= offset &&
      modelTextWithOffsets.endOffset >= offset
    );
  });
  if (eligibleNodes.length == 1) {
    ...
    const eligibleNode = eligibleNodes.find(
      (textWithOffsets) => textWithOffsets[preferredEdge] === offset
    );
    if (!eligibleNode) {
      throw new IllegalArgumentError(
        `no eligble node found for offset ${offset}`
      );
    }
    return eligibleNode;
  } else {
 ...
 */
export default function globalTextRegionToModelRange(
  root: ModelElement,
  regionStart: number,
  regionEnd: number
): ModelRange {
  ...
  const treeWalker = new ModelTreeWalker({
    filter: selectTextAndVoidNodes,
    range,
  });
  const textNodes = buildTextNodeArrayWithOffsets(treeWalker);
  const textForStart: ModelTextWithOffsets = findBestMatchingTextWithOffset(
    textNodes,
    regionStart,
    'startOffset'
  );
  const textForEnd: ModelTextWithOffsets = findBestMatchingTextWithOffset(
    textNodes,
    regionEnd,
    'endOffset'
  );
