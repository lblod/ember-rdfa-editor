...
function selectCurrentSelection() {
  ...
  if (this.currentSelection[0] === this.currentSelection[1] && richNode) {
    ...
    return {
      selections: [{ richNode, range: this.currentSelection }],
      selectedHighlightRange: this.currentSelection,
      collapsed: true,
    };
  } else {
...
function selectHighlight([start, end], options = {}) {
  ...
  if (start > end) {
    throw new Error(
      `Selection ${start}, ${end} with applied offset of ${options.offset} gives an index in which start of region is not before or at the end of the region`
    );
  }
  ...
  while (nextWalkedNodes.length) {
    ...
    for (let node of currentNodes) {
      if (!node.children || node.children.length == 0) {
        if (
          positionInRange(node.start, [start, end]) ||
          positionInRange(node.end, [start, end]) ||
          positionInRange(start, node.region) ||
          positionInRange(end, node.region)
        ) {
...
function selectContext([start, end], options = {}) {
  ...
  if (!['outer', 'inner', 'auto'].includes(options.scope)) {
    throw new Error(
      `Scope must be one of 'outer', 'inner' or 'auto' but is '${options.scope}'`
    );
  }

  if (start > end) {
    throw new Error(
      `Selection ${start}, ${end} gives an index in which start of region is not before or at the end of the region`
    );
  }
  ...
  if (rdfaBlocks.length == 0) return selections;
  ...
  if (
    options.scope == 'outer' ||
    (options.scope == 'auto' && !foundInnerMatch)
  ) {
...
function isMatchingRdfaAttribute(rdfaAttributes, filter, keys) {
  const isMatchingValue = function (rdfaAttributes, key, value) {
    ...
    if (listFilterKeywords.includes(key)) {
      return value.reduce(
        (isMatch, v) =>
          isMatch && (rdfaAttributes[rdfaAttributesKey] || []).includes(v),
        true
      );
    } else {
      if (key == 'resource') {
        return (
          rdfaAttributes['resource'] == value ||
          rdfaAttributes['about'] == value
        );
      } else {
        if (
  ...
  const nonEmptyKeys = keys.filter(
    (key) =>
      filter[key] && (filter[key] instanceof RegExp || filter[key].length)
  );
  return nonEmptyKeys.reduce(
    (isMatch, key) =>
      isMatch && isMatchingValue(rdfaAttributes, key, filter[key]),
    true
  );
}
...
function isMatchingContext(block, filter) {
  ...
  const isMatchingScopeForProperty = function (
    context,
    property,
    resource,
    types
    ...
    while (!matchingTriple && i > 0) {
      i--;
      if (context[i].predicate == property) matchingTriple = context[i];
    }

    const subject = matchingTriple.subject;
    if (resource && subject != resource) return false;

    if (types.length) {
      const typesOfSubject = context
        .filter((t) => t.subject == subject && t.predicate == 'a')
        .map((t) => t.object);
      const matchesAllTypes = types.reduce(
        (isMatch, t) =>
          isMatch &&
          ((t instanceof RegExp && typesOfSubject.match(t)) ||
            (typeof t === 'string' && typesOfSubject.includes(t))),
        true
      );
      if (!matchesAllTypes) return false;
    }
  ...
  if (filter.property.length || filter.datatype) {
    hasAnyFilters = true;
    isMatch = isMatchingRdfaAttribute(
      block.semanticNode.rdfaAttributes,
      filter,
      ['property', 'datatype']
    );
    ...
    if (isMatch && (filter.resource || filter.typeof.length)) {
      ...
      isMatch = isMatchingScopeForProperty(
        block.context,
        filter.property[0],
        filter.resource,
        filter.typeof
...
function filterInner(blocks, filter, [start, end]) {
  ...
  const updateSelections = function (selections, newSelection) {
    const isChildOfExistingSelection = selections.find((selection) =>
      selection.richNode.isAncestorOf(newSelection.richNode)
    );

    if (!isChildOfExistingSelection) {
      const updatedSelections = selections.filter(
        (selection) => !selection.richNode.isDescendentOf(newSelection.richNode)
      );
      updatedSelections.push(newSelection);
      return updatedSelections;
    } else {
      // the newSelection is a child of an existing selection. Nothing should happen.
      return selections;
    }
  };
...
function filterOuter(blocks, filter, [start, end]) {
  ...
  const updateSelections = function (selections, newSelection) {
    const isAncestorOfExistingSelection = selections.find((selection) =>
      selection.richNode.isDescendentOf(newSelection.richNode)
    );

    if (!isAncestorOfExistingSelection) {
      const updatedSelections = selections.filter(
        (selection) => !selection.richNode.isAncestorOf(newSelection.richNode)
      );
      updatedSelections.push(newSelection);
      return updatedSelections;
    } else {
      // the newSelection is an ancestor of an existing selection. Nothing should happen.
      return selections;
    }
  };
