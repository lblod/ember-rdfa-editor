...
function triplesDefinedInResource(resourceUri) {
  let domNodes = scanContexts(this.rootNode)
    .filter((c) => c.context.slice(-1)[0].subject === resourceUri)
    .map((c) => c.semanticNode.domNode);
  ...
  let triples = contexts.reduce((acc, d) => {
    return [
      ...acc,
      ...d.context.slice(d.context.findIndex(isSubject(resourceUri))),
    ]; //remove wrapping context.
  }, []);
