...
import PernetRawEditor, {
  InternalSelection,
} from '@lblod/ember-rdfa-editor/utils/ce/pernet-raw-editor';
import {
  isElement,
  isTextNode,
  ...
  handleMovement(
    ...
    if (oldSelection) {
      ...
      if (
        ...
        oldSelection.startNode.absolutePosition !==
          newSelection.startNode.absolutePosition
      ) {
        // Node is still part of the dom tree.
        const lumpNode = getParentLumpNode(
          nodeBeforeOldSelection,
          editor.rootNode
        );
        if (
          lumpNode &&
          isElement(lumpNode) &&
          lumpNode.hasAttribute('data-flagged-remove')
        ) {
    ...
    if (isInLumpNode(newSelection.startNode.domNode, editor.rootNode)) {
      if (
        ...
        jumpOverLumpNodeBackwards(
          newSelection.startNode.domNode,
          editor.rootNode,
          editor
        );
      } else {
      ...
      editor.setCaret(
        newSelection.startNode.domNode,
        newSelection.startNode.relativePosition
      );
    }
  }
}
...
function jumpOverLumpNodeBackwards(
  node: Node,
  rootNode: HTMLElement,
  editor: PernetRawEditor
) {
