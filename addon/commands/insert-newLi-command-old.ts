...
export default class InsertNewLiCommand extends Command {
  ...
  execute(): void {
    ...
    const firstText = selected.find(node=>ModelNode.isModelText(node));
    const lastText = [...selected].reverse().find(node=>ModelNode.isModelText(node));
    ...
    //hacky fix for the new way offsets are handled(without rewriting the whole thing... really should though)
    if(selection.isCollapsed){
      if(selectionStart.isInsideText()){
        startPos = selectionStart.parentOffset-selectionStart.nodeAfter()?.getOffset();
      }
      else if(selectionStart.nodeBefore() === firstText){
        startPos=firstText.length-1;
      }
      else if(selectionStart.nodeAfter() === firstText){
        startPos=0;
      }
      endPos=startPos;
    }
      ...
      if(selectionEnd.isInsideText()){
        endPos = selectionEnd.parentOffset-selectionEnd.nodeBefore()?.getOffset();
      }
      else{
        endPos = lastText.length-1;
      }
    }
